...
| `deflated.py`                | Full data: Deflated and probabilistic Sharpe of every full-data trial on the test period → `results/deflated_sharpe_{full,official}.csv` |
| `official.py`                | The authors' CRSP residuals (`gregzanotti/dlsa-public`) → `data/full/residuals_official_*.npz` for `official_*` suites |
...
`full_paper_costs`, `full_recipe`, `full_bench`) and on the authors' residuals (`official_paper`,
`official_paper_costs`, `official_recipe`): `docs/kaggle_plan.md` and `docs/full_report.md`. Full pipeline after the suites:
