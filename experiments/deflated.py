...
def trial_returns(results_dir: Path = RESULTS_DIR) -> dict[str, pd.DataFrame]:
    trials = {}
    for path in sorted(results_dir.glob("*/returns.csv")):
        name = path.parent.name
        if not name.startswith(TRIAL_PREFIXES) or not (path.parent / "metrics.json").exists():
            continue
...
def deflate(trials: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for name, frame in trials.items():
        ...
        for kind, column in columns.items():
            ...
            row[f"{kind}_expected_max_sharpe"] = result[
                "expected_maximum_sharpe_under_selection"
            ]
...
def main() -> None:
    frame = deflate(trial_returns())
    frame.to_csv(RESULTS_DIR / "deflated_sharpe_full.csv", index=False)
    print(
    ...
    print(f"\nWrote {RESULTS_DIR / 'deflated_sharpe_full.csv'}")
